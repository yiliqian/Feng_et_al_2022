...
function [para_vec,acceptance_vec,LogPosterior_vec] = AdaptiveMCMC_DynPara_v2(para0,dyn_sys,...
    exp_time,exp_mean,exp_std,num_iti,burn_in,step_ini,k_adapt,step_min)
...
% k_adapt - gain for adapative mcmc
%%%%%%%%%%%%
% Outputs:
...
LogLikelihood_dyn = Dyn_LogLikelihood_SingleExp(para0,dyn_sys,exp_time,exp_mean(1),exp_mean,exp_std);
...
%         para_std = 2.38^2*std(para_vec(1:j-1,:))/num_parameters;
    ...
    LogLikelihood_dyn = Dyn_LogLikelihood_SingleExp(para_current,dyn_sys,exp_time,exp_mean(1),exp_mean,exp_std);
    
    % compute log posterior
    LogPosterior = LogLikelihood_dyn + LogPrior(para_current);
